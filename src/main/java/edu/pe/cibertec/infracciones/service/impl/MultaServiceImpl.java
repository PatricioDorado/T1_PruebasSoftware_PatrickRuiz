...
public class MultaServiceImpl implements IMultaService {
    ...
    public void transferirMulta(Long multaId, Long nuevoInfractorId) {
        ...
        Infractor nuevo = infractorRepository.findById(nuevoInfractorId).orElseThrow(() -> new InfractorNotFoundException(nuevoInfractorId));
        if (nuevo.isBloqueado()) {
        ...
        if (!nuevo.getVehiculos().contains(multa.getVehiculo())) {
            throw new RuntimeException("El vehículo no pertenece al nuevo infractor");
        }
        multa.setInfractor(nuevo);
