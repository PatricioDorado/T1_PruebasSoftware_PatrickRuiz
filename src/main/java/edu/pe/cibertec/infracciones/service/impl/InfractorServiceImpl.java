...
public class InfractorServiceImpl implements IInfractorService {
    ...
    public Double calcularDeuda(Long infractorId) {
        ...
        List<Multa> pendientes = multaRepository.findByInfractor_IdAndEstado(infractorId, EstadoMulta.PENDIENTE);
        List<Multa> vencidas = multaRepository.findByInfractor_IdAndEstado(infractorId, EstadoMulta.VENCIDA);
        double deudaPendiente = pendientes.stream().mapToDouble(Multa::getMonto).sum();
        double deudaVencida = vencidas.stream().mapToDouble(m -> m.getMonto() * 1.15).sum();
        return deudaPendiente + deudaVencida;
    }
    ...
    public void desasignarVehiculo(Long infractorId, Long vehiculoId){
        ...
        List<Multa> pendientes = multaRepository.findByVehiculo_IdAndEstado(vehiculoId, EstadoMulta.PENDIENTE);
        if (!pendientes.isEmpty()) {
            throw new RuntimeException("El vehículo tiene multas pendientes");
        }
        infractor.getVehiculos().remove(vehiculo);
        boolean hayPendientes = multaRepository.findByVehiculo_IdAndEstado(vehiculoId, EstadoMulta.PENDIENTE).size() > 0;
        if (!hayPendientes) {
